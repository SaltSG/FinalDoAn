...
curriculumRouter.get('/:spec', async (req, res) => {
  ...
  let totalCreditsCounted = 0; // counts only courses that contribute to graduation credits
  let totalGpaCredits = 0; // counts only courses that contribute to GPA
  const breakdown: any[] = [];
...
curriculumRouter.post('/:spec/course', async (req, res) => {
  const spec = String(req.params.spec);
  const { semester, code, name, credit, countInGpa, countInCredits } = req.body ?? {};
...
curriculumRouter.put('/:spec/course', async (req, res) => {
  const spec = String(req.params.spec);
  const { code, name, credit, countInGpa, countInCredits } = req.body ?? {};
...
curriculumRouter.post('/seed', async (req, res) => {
  const force = Boolean(req.body?.force);

  // Minimal mapping helper to strip grade/status fields if present
  ...
  // Data copied from frontend samples (structure only)
  const devSemesters = mapSemesters([
    { semester: 'HK1', courses: [
      { code: 'BAS1105M', name: 'Giáo dục quốc phòng', credit: 3, countInCredits: false, countInGpa: false },
      { code: 'BAS1106', name: 'Giáo dục thể chất 1', credit: 2, countInCredits: false, countInGpa: false },
    ...
    { semester: 'HK3', courses: [
      ...
      { code: 'SKD1102', name: 'Kỹ năng làm việc nhóm', credit: 1, countInCredits: false, countInGpa: false },
    ]},
    { semester: 'HK4', courses: [
      { code: 'BAS1122', name: 'Tư tưởng Hồ Chí Minh', credit: 2 },
      { code: 'BAS1159', name: 'Tiếng Anh (Course 3)', credit: 4 },
      { code: 'INT1306', name: 'Cấu trúc dữ liệu và giải thuật', credit: 3 },
      { code: 'INT1325', name: 'Kiến trúc máy tính và hệ điều hành', credit: 2 },
      ...
      { code: 'SKD1103', name: 'Kỹ năng tạo lập Văn bản', credit: 1, countInCredits: false, countInGpa: false },
    ]},
    { semester: 'HK5', courses: [
      ...
      { code: 'MUL1422', name: 'Tổ chức sản xuất sản phẩm đa phương tiện', credit: 2 },
    ...
    { semester: 'HK6', courses: [
      ...
      { code: 'SKD1101', name: 'Kỹ năng thuyết trình', credit: 1, countInCredits: false, countInGpa: false },
  ...
  const designSemesters = mapSemesters([
    // HK1 - HK4 học giống nhau với chương trình dev
    { semester: 'HK1', courses: [
      { code: 'BAS1105M', name: 'Giáo dục quốc phòng', credit: 3, countInCredits: false, countInGpa: false },
      { code: 'BAS1106', name: 'Giáo dục thể chất 1', credit: 2, countInCredits: false, countInGpa: false },
      { code: 'BAS1150', name: 'Triết học Mác - Lênin', credit: 3 },
    ...
    // HK5 trở đi theo chuyên ngành Thiết kế ĐPT (theo ảnh bạn gửi)
    { semester: 'HK5', courses: [
      { code: 'BAS1153', name: 'Lịch sử Đảng cộng sản Việt Nam', credit: 2 },
      { code: 'BAS1160', name: 'Tiếng Anh (Course 3 Plus)', credit: 2 },
      { code: 'MUL13124', name: 'Dựng audio và video phi tuyến', credit: 3 },
      { code: 'MUL13152', name: 'Thiết kế web cơ bản', credit: 3 },
      { code: 'MUL14143', name: 'Mỹ học', credit: 3 },
      { code: 'MUL1415', name: 'Kỹ xảo đa phương tiện', credit: 2 },
      { code: 'MUL1422', name: 'Tổ chức sản xuất sản phẩm đa phương tiện', credit: 2 },
      { code: 'MUL1423', name: 'Kịch bản đa phương tiện', credit: 2 },
    ]},
    { semester: 'HK6', courses: [
      { code: 'MUL14131', name: 'Luật xa gần', credit: 3 },
      { code: 'MUL14132', name: 'Cơ sở tạo hình nâng cao', credit: 3 },
      { code: 'MUL14135', name: 'Thiết kế sản phẩm đa phương tiện', credit: 3 },
      { code: 'MUL14136', name: 'Nghệ thuật đồ họa chữ (Typography)', credit: 3 },
      { code: 'MUL14140', name: 'Thiết kế hình động 2', credit: 3 },
      { code: 'MUL14155', name: 'Thiết kế giao diện người dùng', credit: 3 },
      { code: 'SKD1101', name: 'Kỹ năng thuyết trình', credit: 1, countInCredits: false, countInGpa: false },
    ]},
    { semester: 'HK7', courses: [
      { code: 'MUL14127', name: 'Thiết kế Game', credit: 3 },
      { code: 'MUL14138', name: 'Thiết kế ấn phẩm điện tử 1', credit: 3 },
      { code: 'MUL14144', name: 'Kịch bản phân cảnh', credit: 3 },
      { code: 'MUL14145', name: 'Thiết kế hình động 3D', credit: 3 },
      { code: 'MUL14153', name: 'Thiết kế ứng dụng trên đầu cuối di động', credit: 3 },
    ]},
    { semester: 'HK8', courses: [
      { code: 'MUL13148', name: 'Bản quyền số', credit: 2 },
      { code: 'MUL14139', name: 'Thiết kế sản phẩm điện tử 2', credit: 3 },
      { code: 'MUL14141', name: 'Đồ án thiết kế sản phẩm đa phương tiện', credit: 2 },
      { code: 'MUL14155', name: 'Thiết kế giao diện người dùng', credit: 3 },
      { code: 'MUL14204', name: 'Thực tập chuyên sâu', credit: 4 },
      { code: 'MUL1465', name: 'Chuyên đề thiết kế đa phương tiện', credit: 1 },
      { code: 'SKD1108', name: 'Phương pháp luận nghiên cứu khoa học', credit: 2 },
    ]},
    { semester: 'HK9', courses: [
      { code: 'CDT1434', name: 'Đồ án tốt nghiệp', credit: 6 },
