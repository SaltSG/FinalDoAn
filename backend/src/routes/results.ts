...
// PUT /api/results  body: { userId, data }
resultsRouter.put('/', async (req, res) => {
  ...
  const user = await User.findById(userId);
  if (!user) return res.status(404).json({ message: 'User not found' });
  // If only updating specialization, skip GPA computation
  let semGpa4: Record<string, number> | undefined;
  let cumGpa4: Record<string, number> | undefined;
  const setPayload: any = {};
  if (data && typeof data === 'object') {
    // Compute stats (GPA HK and tích lũy hệ 4) by joining curriculum to get flags & credits
    ...
    // helper: fourFrom10 (use same threshold as frontend)
    ...
    for (const semKey of sortedSemKeys) {
      ...
      for (const code of Object.keys(courses)) {
        ...
        sumW += g4 * credit;
        sumC += credit;
        // accumulate only for passed courses (>= 4.0 system 10)
        if (ov.grade >= 4.0) {
          cumSumW += g4 * credit;
          cumSumC += credit;
        }
...
// DELETE /api/results?userId=... -> clear all overrides for user
resultsRouter.delete('/', async (req, res) => {
  const userId = getUserId(req);
  if (!userId) return res.status(400).json({ message: 'userId required' });
  if (!mongoose.isValidObjectId(userId)) return res.status(400).json({ message: 'invalid userId' });
  const user = await User.findById(userId);
